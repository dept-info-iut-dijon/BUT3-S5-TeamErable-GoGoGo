...
export default (function () {
    ...
    */
    ...
    if (hideSeconds) {
        ...
        // done entering hours, so shift highlight to minutes
        if (
        (charsForHours < 1 && cursorSelection === 'hours') || (sectioned[0].length >= charsForHours && cursorSelection === 'hours')
        ) {
        if (charsForHours < 1) {
            sectioned[0] = '00';
        }
    ...
    // Select all of the input fields with the attribute "duration-picker"
    const getInputFields = document.querySelectorAll('input.duration-picker');
    getInputFields.forEach((inputBox) => {
        ...
        // set css classes
        controlWrapper.setAttribute('class', 'duration-picker-input-controls-wrapper');
        // set inline styles
